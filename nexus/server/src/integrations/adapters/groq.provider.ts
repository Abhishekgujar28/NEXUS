...
export class GroqProvider implements AIProvider {
  ...
  async generate(prompt: string, system?: string, options?: AIGenerateOptions): Promise<string> {
    ...
    try {
      ...
      if (!content) throw new Error('Groq returned empty response payload');
      return content;
    } catch (err) {
  ...
  async generateStructured<T>(prompt: string, system?: string, options?: AIGenerateOptions): Promise<T> {
    ...
    try {
      ...
      logger.warn(`[Groq] Returned malformed JSON`, { snippet: raw.slice(0, 200) });
      throw new AIProviderError({
        provider: this.name,
        model: options?.model || GROQ_MODELS[0],
        message: `Groq returned invalid JSON: ${(err as Error).message}`,
        originalError: err,
      });
    }
  }
  ...
  async embed(text: string): Promise<number[]> {
    logger.warn('[Groq] Embeddings not natively supported by Groq, returning fallback vector');
