  ...
  async search(query: string): Promise<NormalizedSource[]> {
    try {
      const { data } = await safeFetch(
        `https://export.arxiv.org/api/query?search_query=all:${encodeURIComponent(query)}&max_results=10&sortBy=relevance`,
      ...
      logger.error('arXiv search failed', { err: (err as Error).message });
      return [];
    }
  }
}
